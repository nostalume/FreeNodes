"""Bounded Mihomo capability probing with controlled target windows."""
...
from src.mihomo import MihomoValidator, loopback_port
    ...
    async def _probe_blocks(
        ...
        async def close_windows(
            ...
            closing = await asyncio.gather(
                *(observe(self._control_name(target), target) for target in targets)
            )
            return tuple(
                TargetControlWindow(
                    target_id=target.id,
