...
from freenodes.capability import CapableCatalog
from freenodes.config import FrozenModel, RepositoryIdentity
from freenodes.nodes import Node
from freenodes.proxies import VmessUriPayload
...
class SubscriptionURLs(FrozenModel):
    identity: RepositoryIdentity = RepositoryIdentity()

    @classmethod
    def from_identity(cls, identity: RepositoryIdentity) -> SubscriptionURLs:
...
def _site_slugs(catalog: CapableCatalog) -> dict[str, str]:
...
def render_profiles(
    ...
    entries = registry or SubscriptionURLs()
    labels = {
    ...
    uri_lines = []
    for node in (item for item in catalog.nodes if item.kind == "dual"):
        share = (
