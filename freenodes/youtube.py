...
from freenodes.drive import extract_drive_id
...
def extract_date_from_title(title: str, observed_on: date) -> str | None:
    ...
    if match:
        parsed = date(observed_on.year, int(match.group(1)), int(match.group(2)))
        if (parsed - observed_on).days > 30:
