...
class TestExtractDateFromTitle:
    ...
    def test_slash_date_cn(self):
        assert (
            extract_date_from_title("【每日更新】290个免费节点（2026/6/22）")
            == "2026-06-22"
        )

    def test_slash_date_en(self):
        assert (
            extract_date_from_title("[Daily Update] 270 Free Nodes (06/16/2026)")
            == "2026-06-16"
        )
...
class TestExtractExternalLinks:
    ...
    def test_youtube_redirect_extraction(self):
        ...
        links = extract_external_links(text)
        assert len(links) >= 1
        assert any("paste.to" in link for link in links)
...
class TestExtractVideoId:
    def test_standard_url(self):
        assert (
            _extract_video_id("https://www.youtube.com/watch?v=abc123def45")
            == "abc123def45"
        )
    ...
    def test_embed_url(self):
        assert (
            _extract_video_id("https://www.youtube.com/embed/abc123def45")
            == "abc123def45"
        )

    def test_shorts_url(self):
        assert (
            _extract_video_id("https://www.youtube.com/shorts/abc123def45")
            == "abc123def45"
        )
    ...
    def test_with_extra_params(self):
        assert (
            _extract_video_id("https://www.youtube.com/watch?v=abc123def45&t=120")
            == "abc123def45"
        )
