...
import freenodes.profiles as profiles
from freenodes.nodes import SourceArtifact, admit_artifacts
from freenodes.proxies import semantic_fingerprint
...
def test_registry_is_the_single_explicit_import_url_contract():
    registry = profiles.SubscriptionURLs()
    ...
    assert registry.plain.direct.endswith("/HEAD/nodes/merged.txt")


def test_rendered_bundle_has_exact_aggregate_contract_and_safe_site_provider():
    bundle = profiles.render_profiles(sample_catalog(), profiles.SubscriptionURLs())
...
def test_plain_and_base64_v2ray_profiles_are_byte_equivalent():
    ...
    assert plain.startswith(b"trojan://uri-secret@uri.example:443#")
...
def test_provider_profiles_use_http_sources_and_never_yaml_aliases():
    ...
    assert set(direct["proxy-providers"]) == {"uri-source", "yaml-source"}
...
""",
                ...
                SourceArtifact.inline(
                    site="uri",
                    content="trojan://secret@uri.example:443#URI",
                    observed_at=NOW,
                ),
            ],
            now=NOW,
...
def test_consumer_outputs_share_one_canonical_name_and_semantic_identity():
    ...
    catalog = capable_catalog(admit_artifacts(artifacts, now=NOW))
...
def test_unverified_country_text_remains_unknown(label):
    ...
    profile = yaml.safe_load(
        profiles.render_profiles(catalog).files["nodes/merged.yaml"]
    )
