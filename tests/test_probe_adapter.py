"""Behavioral contract for bounded Mihomo capability probing."""
...
from src.probe import MihomoDelayProbe, MihomoProbeSession
...
async def test_session_is_blocked_bounded_capped_and_reaps_one_process(tmp_path):
    ...
    receipt = await session.probe_capabilities(plan(*nodes), TARGETS, QualityPolicy())
...
async def test_invalid_controls_stop_candidates_and_preserve_diagnosis(tmp_path):
    ...
    receipt = await session.probe_capabilities(
        plan(node(1, "one")), TARGETS, QualityPolicy()
    )
...
async def test_deadline_and_cancellation_reap_process_and_workers(tmp_path):
    ...
    async def exercise(cancel: bool):
        ...
        task = asyncio.create_task(
            session.probe_capabilities(plan(node(1, "one")), TARGETS, QualityPolicy())
        )
        await started.wait()
        if cancel:
            task.cancel()
...
async def test_config_rejection_isolated_without_starting_extra_processes(tmp_path):
    ...
    receipt = await session.probe_capabilities(
        plan(node(1, "good"), node(2, "bad")), TARGETS, QualityPolicy()
    )
